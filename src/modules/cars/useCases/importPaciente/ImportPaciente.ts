...
class ImportPaciente {
  constructor(private pacienteRepository: IPacienteRepository) {}

  loadPaciente(file: Express.Multer.File): Promise<IImportPaciente[]> {   // definindo que o retorno vai ser uma promise
    return new Promise((resolve, reject)  => {      // sucesso ou rejeição
        ...
        const paciente: IImportPaciente[] = []; // tipo e recebe um array de IImportCategory
        ...
        parseFile.on("data", async (line) => {
          ...
          const [name, cpf, email, telefone, endereco, genero, idade, rg, tipo] = line; // desestruturação
          paciente.push({
  ...
  async execute(file: Express.Multer.File): Promise <void> {  // como é uma função assincrona, ela precisa retornar uma promise
    const paciente =  await this.loadPaciente(file);   // vai eesperar a finalização da função
    console.log(paciente);
  }
}
export { ImportPaciente };
